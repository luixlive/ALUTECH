...
void PWM_vdfnDriverInit(){
	...
	TPM1_MOD |= TPM_MOD_MOD(1000);				/* Modulo value (0-1000) */
	...
	TPM1_C0SC |= TPM_CnSC_ELSB_MASK;			/* Edge or Level Select */
	//TPM1_CnSC(0) |= TPM_CnSC_MSA_MASK;		/* Channel Mode Select */
	TPM1_C0SC |= TPM_CnSC_MSB_MASK;			/* Channel Mode Select */
	
	TPM1_C1V |= TPM_CnV_VAL(1000); 				/* Channel Value */
}
